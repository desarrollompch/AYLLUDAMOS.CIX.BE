    ...
    <div class="card-body">
        ...
        <div class="row">
            <div class="col-sm-3">
                <div class="form-group">
                    <label for="dni">
                        Documento Identidad - Passaporte
                    </label>
                    <input type="text" class="form-control" name="dni" placeholder="Ingrese Documento Identidad - Passaporte" id="dni" v-model="persona.dni">
        ...
        <div class="row">
            ...
            <div class="col-sm-3">
                <div class="form-group">
                    ...
                    <select name="estado_persona_id" id="estado_persona_id" v-model="persona.estado_persona_id" class="form-control"  v-on:change="changeEstadoPersona()">
                        <option value="">Selecciona</option>
                        <option v-for="estadoPersona in estadoPersonas" :value="estadoPersona.id">{{estadoPersona.descripcion}}</option>
                    </select>
                    <small class="form-text text-danger" v-if="errors.estado_persona_id"
                           v-for="error in errors.estado_persona_id">
                        {{error}}
                    </small>
                </div>
            </div>
...
<script>
    export default {
        ...
        mounted() {
            ...
            axios.get(route('tipo-persona.all-sin-paginado')).then((response) => {
            ...
            axios.get(route('persona.getEstadoPersona')).then((response) => {
                this.estadoPersonas = response.data.estadopersona;
        ...
        methods: {
            changeEstadoPersona(){
                for (let row of this.estadoPersonas) {
                    if (row.id == this.persona.estado_persona_id) {
                        this.persona.state = row.descripcion;
                    }
                }
            },
